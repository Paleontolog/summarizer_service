...
def add_special_tokens(model_path="sberbank-ai/rugpt3large_based_on_gpt2"):
    ...
    with torch.no_grad():
        ...
        for _ in tnrange(length - 1):
            ...
            for j in range(len(sequences)):
                new_generated = torch.cat((context,
                                           torch.tensor([sequences[j]],
                                                        dtype=torch.long,
                                                        device=device)),
                                          dim=1)
...
def generate_sample(data, tokenizer, model, num=1, eval_step=False,
                    length=100, temperature=1, top_k=10, top_p=0.5,
